...
description: Real-time monitoring and updates for Trigger.dev v4 runs including authentication (public tokens, trigger tokens), backend run subscriptions, realtime streams v2, React frontend hooks (@trigger.dev/react-hooks), subscribing to runs/tags/batches, streaming data, wait tokens, and run object properties. Use when building real-time dashboards, monitoring task progress from frontend, subscribing to run status changes, streaming AI output, or creating interactive task workflows with user approvals.
---
